...
mongoose.connect('mongodb+srv://<realmID>:<realmPassword>@atlascluster.cinwufg.mongodb.net/<userData>'
).then(() => {
...
const GOOGLE_PROJECT_ID = 'project-id';
...
app.get('/auth/facebook/callback', async (req, res) => {
  ...
  try {
    ...
    .then((res) => {
      vueAppUrl.value = `http://localhost:5173/home?userId=${profileData.id}&email=${profileData.email}&name=${profileData.name}&pictureUrl=${res.profile.pictureUrl + "&access_token=" + accessToken}`;
    })
    console.log(realmUserfb);
    return res.redirect(vueAppUrl?.value, '_blank', 'width=600,height=800');
  } catch (error) {
    console.error(error);
